...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    // Access control: Non-admin/manager must be a project member to upload files
    ...
    const result = await sql`
      INSERT INTO project_files (project_id, file_name, file_url, uploaded_by)
      VALUES (${projectId}, ${fileName}, ${fileUrl}, ${session.userId})
      RETURNING *
    `;
    ...
    return NextResponse.json({ error: 'Failed to add project file' }, { status: 500 });
  }
}
